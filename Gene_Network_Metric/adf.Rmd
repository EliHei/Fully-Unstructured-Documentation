...
g <- ggm1(data = data.frame(t(tusi[hvg_sym,])), rho = 0.3)
g$graph$network
t.matrix <- abs(g$wi)
...
write.csv(as.matrix(d.matrix), "../results/gmm_htseq_tusi_.csv")
...
dim.reduce(data = d.matrix, method = "tsne", annot1 = strsplit2(colnames(htseq), "_")[,1])
```
...
write.csv(as.matrix(d.matrix), "../results/ggm_htseq_loop_.csv")
d.matrix <- read.csv("../results/ggm_htseq_loop_.csv", row.names = 1)
