...
```{r}
d.matrix <- dist.matrix(exprMat[hvg,], k = 5)
d.matrix <- dist(t(exprMat))
# write.csv(as.matrix(d.matrix), "../results/ggm.csv")
...
dm <- DiffusionMap(data = data.frame(sample = guo$Cell, col = factor(cell_types)), distance = as.dist(d.matrix))
dpt <- DPT(dm)
