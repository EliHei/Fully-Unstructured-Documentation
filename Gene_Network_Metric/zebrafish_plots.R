...
p1 <- plot.DPT(dpt, dcs = c(1, 2, 3), col = factor(c(rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400))))
df <- data.frame(DPT1 = dpt$DC1, DPT2 = dpt$DC2, col = dpt$col)
p2 <- ggplot(df, aes(x = DPT1, y = DPT2, color = col)) + geom_point() + theme_bw()
plot.DPT(dpt, dcs = c(1, 2))

df <- data.frame(DPT2 = dpt$DC2, DPT3 = dpt$DC3, col = dpt$col)
p3 <- ggplot(df, aes(x = DPT2, y = DPT3, color = col)) + geom_point() + theme_bw()
plot.DPT(dpt, dcs = c(2, 3))

df <- data.frame(DPT1 = dpt$DC1, DPT3 = dpt$DC3, col = dpt$col)
p4 <- ggplot(df, aes(x = DPT1, y = DPT3, color = col)) + geom_point() + theme_bw()
...
ind <-c(hpf_sp[1:400], hpf_nc[1:400], hpf_pa[1:400], hpf_sm[1:400])
classes <- c(rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400))
d.matrix <- dist(t(df[ind]))

dm <- DiffusionMap(data = data.frame(sample = colnames(hpf)[ind], col = factor(classes)), distance = as.dist(d.matrix))
rm(d.matrix)

dpt <- DPT(dm)
p5 <- plot.DPT(dpt, dcs = c(1, 2, 3), col = factor(c(rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400))))
df <- data.frame(DPT1 = dpt$DC1, DPT2 = dpt$DC2, col = dpt$col)
p6 <- ggplot(df, aes(x = DPT1, y = DPT2, color = col)) + geom_point() + theme_bw()
plot.DPT(dpt, dcs = c(1, 2))

df <- data.frame(DPT2 = dpt$DC2, DPT3 = dpt$DC3, col = dpt$col)
p7 <- ggplot(df, aes(x = DPT2, y = DPT3, color = col)) + geom_point() + theme_bw()
plot.DPT(dpt, dcs = c(2, 3))

df <- data.frame(DPT1 = dpt$DC1, DPT3 = dpt$DC3, col = dpt$col)
p8 <- ggplot(df, aes(x = DPT1, y = DPT3, color = col)) + geom_point() + theme_bw()
plot.DPT(dpt, dcs = c(1, 3))




DPT.fig <- ggarrange(p2, p3, p4, p6, p7, p8, nrow= 2, ncol=3, common.legend = TRUE, legend="right", labels = c("GNM", "", "", "Euclidean", "", ""))
...
graph.col.purple <- function(graph, lay, grp, sz){
  igraph::V(graph)$color <- rgb(floor(255*(grp^4)/max((grp^4))), 0, floor(255*(grp^4)/max((grp^4))), maxColorValue=255, alpha=255)
  v <-  igraph::V(graph)
  graph <- as.undirected(graph)
  igraph::V(graph)$size <- sz
  iso <- function(graph, mode = 'all') {
...
# CMP CD41
graph.col.red(graph = l, lay= lay, grp = (as.numeric(nc_groups) + 1), sz = nc_size)
title("Neural Crest",cex.main=1,col.main="black")
# CMP Flt3+ Csf1r+
graph.col.green(graph = l, lay= lay, grp = (as.numeric(pa_groups) + 1), sz = pa_size)
title("Pharyngeal Arch",cex.main=1,col.main="black")
# CMP Irf8-GFP+ MHCII+
graph.col.blue(graph = l, lay= lay, grp = (as.numeric(sm_groups) + 1), sz = sm_size)
title("Somatic Mesoderm",cex.main=1,col.main="black")

# CMP Irf8-GFP+ MHCII+
graph.col.purple(graph = l, lay= lay, grp = (as.numeric(sp_groups) + 1), sz = sp_size)
title("Spinal Cord",cex.main=1,col.main="black")
