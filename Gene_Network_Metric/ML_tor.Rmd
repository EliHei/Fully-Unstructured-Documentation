...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
hvg <- hv.genes(tusi, 500)
hvg <- rownames(tusi)[hvg]
...
#hvg <- hvg[!(hvg %in% outs)]
df <- tusi_bbm[hvg,]
# outs <- which(rowSds(data.matrix(df)) == 0)
# hvg <- hvg[-outs]
...
training <- df[train, ]
testing <- df[-train, ]
cv1 <- cv.glmnet(x = data.matrix(df), y = factor(cell_types[ind]), family = "binomial", nfold = 20, paralle = TRUE, alpha = 0.8)
md1 <- glmnet(data.matrix(training), factor(cell_types[ind][train]), family = "binomial", lambda = cv1$lambda.1se, alpha = 0.8)
...
write.csv(as.matrix(d.matrix), "../results/hvg_cyto.csv")
...
df <- data.frame(DPT1 = dpt$DC3, DPT2 = dpt$DC4, col = dpt$col)
ggplot(df, aes(x = DPT1, y = DPT2, color = col)) + geom_point()
