...
ids <- c(nc.ids, pa.ids, sp.ids, sm.ids, ms.ids, gl.ids)
```
...
df08 <- df_temp[,hpf08.read]
df10 <- df_temp[,hpf10.read]
df14 <- df_temp[,hpf14.read]
df18 <- df_temp[,hpf18.read]
df24 <- df_temp[,hpf24.read]

groups_08 <- apply(df08, 1, mean)
size_08 <- apply(df08, 1, function(x) (1/1+log(sd(x))))
groups_10 <- apply(df10, 1, mean)
size_10 <- apply(df10, 1, function(x) (1/1+log(sd(x))))
groups_14 <- apply(df14, 1, mean)
size_14 <- apply(df14, 1, function(x) (1/1+log(sd(x))))
groups_18 <- apply(df18, 1, mean)
size_18 <- apply(df18, 1, function(x) (1/1+log(sd(x))))
groups_24 <- apply(df24, 1, mean)
size_24 <- apply(df24, 1, function(x) (1/1+log(sd(x))))
...
lay <- layout_nicely(l)
# hpf08
graph.col(graph = l, lay= lay, grp = (as.numeric(groups_08) + 1), sz = size_08)
# hpf10
graph.col(graph = l, lay= lay, grp = (as.numeric(groups_10) + 1), sz = groups_10)
# hpf14
graph.col(graph = l, lay= lay, grp = (as.numeric(groups_14) + 1), sz = groups_14)
# hpf18
graph.col(graph = l, lay= lay, grp = (as.numeric(groups_18) + 1), sz = groups_18)
# hpf24
graph.col(graph = l, lay= lay, grp = (as.numeric(groups_24) + 1), sz = groups_24)
