...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
# df <- readxl::read_excel("../data/new_batch/GSE61533_HTSEQ_count_results.xls")

hpf <- read.table("../data/new_batch/GSE81682_HTSeq_counts.txt", header = T)
...
rownames(hpf) <- hpf$ID
hpf <- hpf[,-1]
# cell_types <- colnames(cell_types)[which()]
```


```{r message=FALSE, warning=FALSE, paged.print=FALSE}
hpf <- muvis::data_preproc(hpf)
hpf <- apply(hpf, 2, normalize2)
# hpf <- read.table("/Users/elihei/Downloads/nestorowa_corrected_log2_transformed_counts.txt")
# hpf <- t(hpf)
hpf <- data.frame(hpf)
cell_types <- is.one[colnames(hpf)]
loop_types <- which(cell_types %in% c("CMP_broad", "GMP_broad", "LMPP_broad", "MPP1_broad", "MPP2_broad", "MPP3_broad"))
...
# write.csv(as.matrix(d.matrix), "../results/ggm_005_new_batch_loop1.csv")
d.matrix <- read.csv("../results/ggm_005_new_batch_loop1.csv", row.names = 1)
...
# df <- data.frame(df)
# d.matrix <- dist.matrix(df[loop_types], m = 1, k = 5)
# write.csv(as.matrix(d.matrix), "../results/ggm_01_new_batch2_loop1.csv")
d.matrix <- read.csv("../results/ggm_01_new_batch_loop1.csv", row.names = 1)
...
# Rho = 0.2, #HVG = 300
```{r}
g <- muvis::min_forest(data = data.frame(t(df)))
g$graph$network
l <- g$graph$graph
t.matrix <- g$wi
t.matrix <- scale(t.matrix, center=FALSE, scale=colSums(t.matrix))
df <- data.frame(df)
d.matrix <- dist.matrix(df[loop_types], m = 1, k = 20)
write.csv(as.matrix(d.matrix), "../results/ggm_03_new_batch2_loop1.csv")
d.matrix <- read.csv("../results/ggm_03_new_batch2_loop1.csv", row.names = 1)
...
plot.DPT(dpt, dcs = c(2, 11), col = factor(cell_types[loop_types]))
```


# Rho = 0.15, #HVG = 300
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
g <- ggm1(data = data.frame(t(df)), rho = 0.15)
g$graph$network
# l <- g$graph$graph
# t.matrix <- g$wi
# t.matrix <- scale(t.matrix, center=FALSE, scale=colSums(t.matrix))
# df <- data.frame(df)
# d.matrix <- dist.matrix(df[loop_types], m = 1, k = 5)
# write.csv(as.matrix(d.matrix), "../results/ggm_015_new_batch_loop1.csv")
d.matrix <- read.csv("../results/ggm_015_new_batch_loop1.csv", row.names = 1)
...
write.csv(as.matrix(d.matrix), "../results/eu_new_batch2_loop1.csv")
d.matrix <- read.csv("../results/eu_new_batch2_loop1.csv", row.names = 1)
...
```{r}
new_cyto <- read.table("/Users/elihei/Downloads/coordinates_gene_counts_flow_cytometry.txt")
