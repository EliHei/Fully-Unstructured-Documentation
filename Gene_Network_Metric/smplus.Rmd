...
ids <- c(nc.ids, pa.ids, sp.ids, sm.ids, ms.ids)
```
...
hpf_id <- c(hpf_nc, hpf_pa, hpf_sp, hpf_sm, hpf_ms)
```
...
hv.genes <- function(df, n = 100){
  ...
  sds <- oompaBase::matrixVar(as.matrix(df), xmean = means, na.rm = T)
  lm <- lm(sds~means)
  sm <- summary(lm)
  res <- residuals(lm)
...
# hpf <- data.matrix(hpf)
# hpf <- sqrt(hpf)
# hpf <- scale(hpf, center = FALSE, scale = TRUE)
# hpf <- data.frame(hpf)
