...
l <- g$graph$graph
t.matrix <- g$wi
write.csv(t.matrix, "../data/adj_Shayan.csv")
write.csv(df[,c(hpf_sp, hpf_nc, hpf_pa, hpf_sm)], "../data/data_Shayan.csv")
df <- data.frame(id = c(hpf_sp, hpf_nc, hpf_pa, hpf_sm), class = c(rep("spinal chord", length(hpf_sp)), rep("neural crest", length(hpf_nc)),rep("pharyngal arch", length(hpf_pa)),rep("somatic meso", length(hpf_sm))))
write.csv(df, "../data/classes.csv")



emb <- read.csv("/Users/elihei/Downloads/graph_embedding2.csv", row.names = 1, header = T)

embs <- Rtsne::Rtsne(dist(emb), dims = 2, perplexity = 30, pca = TRUE, max_iter = 1000, is_distance = T)

dist(emb)

plot(embs$Y, col = factor(df$class))
...
d.matrix <- dist.matrix(hpf[hvg, c(hpf_sp[1:400], hpf_nc[1:400], hpf_pa[1:400], hpf_sm[1:400])], m = 1, k = 10)
...
dm <- DiffusionMap(data = df[, c(hpf_sp[1:400], hpf_nc[1:400], hpf_pa[1:400], hpf_sm[1:400])], distance = as.dist(d.matrix))
...
genes <- sample(1:dim(hpf)[1], 1000)
outs <- which(rowSds(hpf[genes,]) == 0)
genes <- genes[-outs]
g <- ggm1(data = data.frame(t(hpf[genes,])), rho = 0.05)
# g$graph$network
l <- g$graph$graph
...
d.matrix <- dist.matrix(hpf[hvg, c(hpf_sp[1:400], hpf_nc[1:400], hpf_pa[1:400], hpf_sm[1:400])], m = 1, k = 5)
# 
# 
write.csv(as.matrix(d.matrix), "../results/Fucking_ggm.csv")
...
df <- hpf[hvg, ]
g <- ggm1(data = data.frame(t(hpf[hvg, c(hpf_sp[1:400], hpf_nc[1:400], hpf_pa[1:400], hpf_sm[1:400])])), rho = 0.2)
# g$graph$network
l <- g$graph$graph
t.matrix <- g$wi
t.matrix <- scale(t.matrix, center=FALSE, scale=colSums(t.matrix))
