...
graph.col.blue <- function(graph, lay, grp, sz){
...
graph.col.red <- function(graph, lay, grp, sz){
  igraph::V(graph)$color <- rgb(floor(255*(grp^4)/max((grp^4))), maxColorValue=255, alpha=255, 0, 0)
  v <-  igraph::V(graph)
  graph <- as.undirected(graph)
  igraph::V(graph)$size <- sz
  iso <- function(graph, mode = 'all') {
...
d.matrix <- read.csv("../results/ggm_0.3.csv", row.names = 1)
...
df <- data.frame(DC1 = dpt$DC1, DC2 = dpt$DC2, cell_type = dpt$col)
p1 <- ggplot(df, aes(x = DC1, y = DC2, color = cell_type)) + geom_point() + theme_pubr()
plot.DPT(dpt, dcs = c(1, 2))

df <- data.frame(DC2 = dpt$DC2, DC3 = dpt$DC3, cell_type = dpt$col)
p2 <- ggplot(df, aes(x = DC2, y = DC3, color = cell_type)) + geom_point() + theme_pubr()
plot.DPT(dpt, dcs = c(2, 3))

df <- data.frame(DC1 = dpt$DC1, DC3 = dpt$DC3, cell_type = dpt$col)
p3 <- ggplot(df, aes(x = DC1, y = DC3, color = cell_type)) + geom_point() + theme_pubr()
plot.DPT(dpt, dcs = c(1, 3))


# d.matrix <- read.csv("../results/euc.csv", row.names = 1)
df <- ido[hvg, ]
d.matrix <- dist(t(ido[hvg, CMP]))

dm <- DiffusionMap(data = data.frame(sample = rownames(ido)[CMP], col = factor(meta_data$Batch_desc[CMP])), distance = as.dist(d.matrix))
dpt <- DPT(dm)

df <- data.frame(DC1 = dpt$DC1, DC2 = dpt$DC2, cell_type = dpt$col)
p4 <- ggplot(df, aes(x = DC1, y = DC2, color = cell_type)) + geom_point() + theme_pubr()
plot.DPT(dpt, dcs = c(1, 2))

df <- data.frame(DC2 = dpt$DC2, DC3 = dpt$DC3, cell_type = dpt$col)
p5 <- ggplot(df, aes(x = DC2, y = DC3, color = cell_type)) + geom_point() + theme_pubr()
plot.DPT(dpt, dcs = c(2, 3))

df <- data.frame(DC1 = dpt$DC1, DC3 = dpt$DC3, cell_type = dpt$col)
p6 <- ggplot(df, aes(x = DC1, y = DC3, color = cell_type)) + geom_point() + theme_pubr()
plot.DPT(dpt, dcs = c(1, 3))

DPT.fig <- ggarrange(p4, p5, p6, p1, p2, p3, nrow= 2, ncol=3, common.legend = TRUE, legend="right", labels = c("Euclidean", "", "", "GNM", "", ""))

annotate_figure(DPT.fig, top = text_grob("Ido dataset: CMP cell type", face = "bold"))

df_temp <- data.matrix(ido[hvg_, ])
df_temp <- ifelse(df_temp > 0, 1, 0)
df_temp <- data.frame(df_temp)
CMP <- which(grepl("^CMP", meta_data$Batch_desc))
CMP <- meta_data$Batch_desc[CMP]
cd41.df <- df_temp[,which(CMP == "CMP CD41")]
flt3.df <- df_temp[,which(CMP == "CMP Flt3+ Csf1r+")]
Irf8.df <- df_temp[,which(CMP == "CMP Irf8-GFP+ MHCII+")]

cd41_groups <- apply(cd41.df, 1, mean)
cd41_size <- apply(cd41.df, 1, function(x) (1/1+(sd(x)))^4)
flt3_groups <- apply(flt3.df, 1, mean)
flt3_size <- apply(flt3.df, 1, function(x) (1/1+(sd(x)))^4)
Irf8_groups <- apply(Irf8.df, 1, mean)
Irf8_size <- apply(Irf8.df, 1, function(x) (1/1+(sd(x)))^4)
...
# CMP CD41
graph.col.red(graph = l, lay= lay, grp = (as.numeric(cd41_groups) + 1), sz = cd41_size)
title("CMP CD41",cex.main=1,col.main="black")
# CMP Flt3+ Csf1r+
graph.col.red(graph = l, lay= lay, grp = (as.numeric(flt3_groups) + 1), sz = flt3_size)
title("CMP Flt3+ Csf1r+",cex.main=1,col.main="black")
# CMP Irf8-GFP+ MHCII+
graph.col.red(graph = l, lay= lay, grp = (as.numeric(Irf8_groups) + 1), sz = Irf8_size)
title("CMP Irf8-GFP+ MHCII+",cex.main=1,col.main="black")
