...
# 
# # d.matrix <- dist.matrix(df[, c(hpf_sp, hpf_nc, hpf_pa, hpf_sm)], m = 1, k = 10)
# # 
# # 
# # heat.map <- Heatmap(
# #   d.matrix,
...
# # )  +
# #   Heatmap(
# #     c(rep("spinal cord", length(hpf_sp)), rep("neural crest", length(hpf_nc)), rep("pharyngeal arch", length(hpf_pa)), rep("somatic meso", length(hpf_sm))),
# #     name = "clusters",
# #     width = unit(5, "mm"))
# # save(d.matrix, "distance_matrix.rda")
# # heat.map
# 
...
# )  +
#   Heatmap(
#     c(rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400)),
#     name = "clusters",
#     width = unit(5, "mm"))
# save(d.matrix, "distance_matrix.rda")
# heat.map
...
# library(Matrix)
# library(plotly)
# d.matrix <- as.dist(d.matrix)  
# d <- dist(hpf[1:1000,1:1000])
# dm <- DiffusionMap(data = hpf[, c(hpf_sp[1:400], hpf_nc[1:400], hpf_pa[1:400], hpf_sm[1:400])], distance = as.dist(d.matrix))
# dpt <- DPT(dm)
# p <- plot.DPT(dpt)
# 
# 
# dm <- DiffusionMap(data = hpf[1:10000, 1:100], distance = "")
# dists <- dist(t(hpf[, c(hpf_sp[1:400], hpf_nc[1:400], hpf_pa[1:400], hpf_sm[1:400])]), method = "euclidean")
# dm <- DiffusionMap(data.frame(covars = c(hpf_sp[1:400], hpf_nc[1:400], hpf_pa[1:400], hpf_sm[1:400])), distance = dists)
# 
# dpt <- DPT(dm)
...
# plot.DiffusionMap(dm, col = factor(c(rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 800))))
# plot.DiffusionMap(dm, col = factor(c(rep("spinal cord", 800), rep("pharyngeal arch", 400), rep("somatic meso", 400))))
# 
# dpt <- DPT(dm, col_by = factor(c(rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400))))
# 
# dpt <- DPT(dm, col_by = factor(c(rep("spinal cord", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400))))
# plot.DPT(dpt, dcs = c(2,3), col = factor(c(rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400))))
# dm <- DiffusionMap(data = hpf[, 1:100])
# 
# find_sigmas(hpf[, 1:1000])
# 
```
...
heat.map <- Heatmap(
  ...
  Heatmap(
    c(rep("germline", 30), rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400), rep("epidermal", 400), rep("endoderm", 400)),
    name = "clusters",
    width = unit(5, "mm"))
...
dpt <- DPT(dm, col = factor(c(rep("germline", 30), rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400), rep("epidermal", 400), rep("endoderm", 400))))

plot.DPT(dpt, dcs = c(1, 2, 3))
