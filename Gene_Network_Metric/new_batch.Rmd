...
hpf <- read.csv("../data/new_batch/GSE61533_HTSEQ_count_results/Sheet1-Table 1.csv")
...
dpt <- DPT(dm)
col <- rep("NA", dim(hpf)[2])
coler <- rbind(col, hpf)
coler[1,] <- is.one[colnames(hpf)]
plot.DPT(dpt, dcs = c(1, 2, 3), col = factor(is.one[colnames(hpf)]))
plot.DPT(dpt, dcs = c(1, 2), col = factor(is.one[colnames(hpf)]))
plot.DPT(dpt, dcs = c(2, 3), col = factor(is.one[colnames(hpf)]))
