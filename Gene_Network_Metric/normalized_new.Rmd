...
```{r}
g <- ggm1(data = data.frame(t(df)), methods = "threshold", threshold = 0.1)
g$graph$network
l <- g$graph$graph
t.matrix <- g$PC
# t.matrix[t.matrix < 0.05] <- 0
# diag(t.matrix) <- 0
# t.matrix <- scale(t.matrix, center=FALSE, scale=colSums(t.matrix))
# df <- data.frame(df)
# d.matrix <- dist.matrix(df[loop_types], m = 1, k = 5)
# write.csv(as.matrix(d.matrix), "../results/PC_005_.csv")
# d.matrix <- read.csv("../results/PC_005_.csv", row.names = 1)

t.matrix <- scale(t.matrix, center=FALSE, scale=colSums(t.matrix))
df <- data.frame(df)
d.matrix <- dist.matrix(df, m = 1, k = 10)
write.csv(as.matrix(d.matrix), "../results/PC_all.csv")
d.matrix <- read.csv("../results/PC_all.csv", row.names = 1)
...
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
g <- ggm1(data = data.frame(t(hpf[hvg,])), rho = 0.01, threshold = 0.1, methods = )
g$graph$network
# l <- g$graph$graph
# t.matrix <- g$wi
# t.matrix <- scale(t.matrix, center=FALSE, scale=colSums(t.matrix))
