...
S <- stats::cov.wt(t(df), method = "ML")$cov
PC <- gRbase::cov2pcor(S)
diag(PC) <- 0
PC[PC < 0.05] <- 0
t.matrix <- PC
...
write.csv(as.matrix(d.matrix), "../results/Zebra_PC_.csv")
d.matrix <- read.csv("../results/Zebra_PC_.csv", row.names = 1)
...
rm(d.matrix)
dpt <- DPT(dm)
plot.DPT(dpt, dcs = c(1, 3, 5), col = factor(c(rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400))))
...
plot.DPT(dpt, dcs = c(1, 5), col = factor(c(rep("spinal cord", 400), rep("neural crest", 400), rep("pharyngeal arch", 400), rep("somatic meso", 400))))
