...
datasets <-
  list.files("/Volumes/elihei/Internship/TP53ness_180716_/data/Preprocessed/") %>% purrr::map(function(f)
    read.table(
...
row.indices <- Reduce(intersect, datasets %>% purrr::map(rownames))
feature.df <- do.call(cbind, datasets %>% purrr::map(function(x) x[row.indices,]))
...
g1 <- which(feature.df$TP53 == 1 & feature.df$Nutlin.3a < 0.9)
g2 <- which(feature.df$TP53 == 1 & feature.df$Nutlin.3a > 0.9)
div(feature.df, g1, g2) -> kl.divvv
imp <- rownames(kl.divvv)[order(desc(kl.divvv$KL))]
